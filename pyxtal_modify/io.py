...
class structure_from_ext():
    ...
    def __init__(self, struc, ref_mols, tol=0.2, ignore_HH=False, add_H=False, hn=None):
        ...
        for i in ref_mols:
            ...
            # Use regex to find all occurrences of the pattern "Letter + Number"
            pattern = re.findall(r'([A-Za-z]+)(\d+)', clean_string)

            # Build the list by repeating each element according to its count
            output_list = [element for elem, count in pattern for element in [elem] * int(count)]
            mol_ele_list.append(output_list)
        # print(mol_ele_list)
...
def search_molecules_in_crystal(struc, tol=0.2, mol_list=None, once=False, ignore_HH=True):
    """
    ...
    def extract_element(symbol_with_number):
        # Use a regular expression to find alphabetic characters
        match = re.match(r"[A-Za-z]+", symbol_with_number)
        if match:
            return match.group()  # Returns the matched string which is the element symbol
        return None  # Return None or raise an error if no match is found

    
    def check_one_site(struc, site0, visited, rmax=2.8):
        ele_species = extract_element(str(site0.species))
        # print(f"ele_species:{ele_species}")
        
        for ele_list in mol_list:
            if ele_species in ele_list:
                mol_select = ele_list
        # print(f"mol_select:{mol_select}")
        if mol_list is not None:
            neigh_sites_initial = struc.get_neighbors(site0, rmax)
            neigh_sites = []
            # print(f"neigh_sites_before:{neigh_sites_initial}")
            for i, mol_py in enumerate(neigh_sites_initial):
                # print(f"mol_py:{mol_py}")
                if extract_element(str(mol_py.species)) in mol_select and len(mol_select) > 1:
                    neigh_sites.append(mol_py)

        else:
            neigh_sites = struc.get_neighbors(site0, rmax)

        # print(f"neigh_sites:{neigh_sites}")
        # print("----------------------------")
        ids = [m.index for m in visited]
        sites_add = []
        ids_add = []
        pbc = isinstance(struc, Structure)
        # print(f"site0:{site0.species}")
        # print(type(site0))
        # print(f"neigh_sites:{neigh_sites}")
...
#     def check_one_site(struc, site0, visited, rmax=2.8):
#         # single = ['Li1']
#         # if str(site0.species) in single:
#         #     neigh_sites = []
#         # else:
#         neigh_sites = struc.get_neighbors(site0, rmax)
#         ids = [m.index for m in visited]
...
#         return sites_add, visited
    ...
    for id, site in enumerate(struc.sites):
        # print(f"site:{site}")
        if id not in visited_ids:
            # print(f"visited_ids:{visited_ids}")
            first_site = site
            visited = [first_site]
            first_site.index = id
